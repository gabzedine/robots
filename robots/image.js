...
async function robot() {
    ...
    async function fetchImageOfAllSentences(content) {
        let thisKeyword = ''
        let arrSearchTerm = content.searchTerm.split(" ")
        for (const sentence of content.sentences) {
            if (sentence.keywords[0].indexOf(arrSearchTerm[0])) {
                thisKeyword = sentence.keywords[0]
            } else if (sentence.keywords[1].indexOf(arrSearchTerm[0])) {
                thisKeyword = sentence.keywords[1]
            } else if (sentence.keywords[2].indexOf(arrSearchTerm[0])) {
                thisKeyword = sentence.keywords[2]
            } else {
                thisKeyword = sentence.keywords[3]
            }

            const query = `${content.searchTerm} ${thisKeyword}`;
